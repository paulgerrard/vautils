...
def generatePrompts(goal, client):
    #
    #   first, load the compaines found in the inbox
    #
    tagsfile = inboxdir+'/tags.export'
    if exists(tagsfile):
        fp=open(tagsfile)
    ...
    for t in tags:
        ...
        try:
            ...
            chat_result = client.contextual_completions.chat_completion(
                messages=[{"role": "user", "content":  PROMPTS[tags[t]['tag']] + term}])
            print(chat_result.choices[0].message.content)
            tagprompts[tags[t]['id']] = chat_result.choices[0].message.content
            
        except Exception as conterror:
            print('Error is:', conterror)

    tpfile = outboxdir + '/tagprompts.json'
    tpjson = json.dumps(tagprompts)
    fp = open(tpfile,'w')
    fp.write(tpjson)
    fp.close()
